...
import { canEditTask, canDeleteTask, canChangeStatus } from "@/lib/permissions";
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  if (canEditTask(user, teamId)) {
    // 전체 필드 수정 가능
  } else if (statusOnly && canChangeStatus(user, teamId, assigneeIds)) {
...
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  ...
  if (!canDeleteTask(user, String(task.teamId))) {
